...
Pour transformer la notion subjective de "désirabilité" en une donnée quantitative exploitable, j'ai mis au point une formule de calcul pondérée. Ce score est calculé pour chaque produit selon la logique suivante :

$$Score = (0.5 \times Like_{norm}) + (0.3 \times Posts_{norm}) + (0.1 \times Color_{score}) + (0.1 \times Material_{score})$$

#### Pourquoi cette pondération ?

- **Les Likes (50%)** : C'est l'indicateur le plus fort. Un "like" sur Vestiaire Collective représente une intention d'achat ou un intérêt direct du consommateur.
...
Cette approche permet de classer les produits non plus sur une impression, mais sur un indicateur composite . Ce score devient ainsi la "variable cible" que mon futur modèle de Machine Learning tentera de prédire pour les nouveaux produits.
